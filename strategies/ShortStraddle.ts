...
export type ShortStraddlePayoff = {
...
export function calculateShortStraddle(
  ...
  if (!chain.data.length) {
    throw new Error(
      "Cannot calculate a short straddle from an empty option chain",
    );
  }
  ...
  // Premiums received by selling the Call and Put
  ...
  const payoffTable: ShortStraddlePayoff[] = nearbyStrikes.map(
    ({ strike_price: closingPrice }) => {

      // Short Call P&L
      const callPl =
        callPremium -
        Math.max(closingPrice - atm.strike_price, 0);

      // Short Put P&L
      const putPl =
        putPremium -
        Math.max(atm.strike_price - closingPrice, 0);
