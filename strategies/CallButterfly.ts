...
export type CallButterflyPayoff = {
...
export function calculateCallButterfly(
  chain: OptionChainResponse,
  recordsEachSide = 10,
) {
  if (!chain.data.length) {
    throw new Error(
      "Cannot calculate a call butterfly from an empty option chain",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Not enough option strikes to construct a call butterfly",
    );
  }
  ...
  if (strikeWidthLower !== strikeWidthUpper) {
    throw new Error(
      "Selected strikes are not equally spaced for a balanced call butterfly",
    );
  }
  ...
  if (netDebit <= 0) {
    throw new Error(
      "Call butterfly does not produce a valid net debit",
    );
  }
  ...
  const payoffTable: CallButterflyPayoff[] =
    nearbyStrikes.map(
