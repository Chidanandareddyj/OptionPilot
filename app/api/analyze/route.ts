...
export async function POST(request: Request) {
  try {
    ...
    const bearCallSpread =
      calculateBearCallSpread(options, 10);

    const bearPutSpread =
      calculateBearPutSpread(options, 10);

    const longStrangle =
      calculateLongStrangle(options, 10);

    const shortStraddle =
      calculateShortStraddle(options, 10);

    const shortStrangle =
      calculateShortStrangle(options, 10);
